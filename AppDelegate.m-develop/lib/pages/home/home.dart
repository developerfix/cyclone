...
import 'package:Siuu/pages/message/screens/chat/chat_page.dart';
...
class OBHomePageState extends State<OBHomePage> with WidgetsBindingObserver {
  ...
  void initState() {
    super.initState();
    OBHomePage.generalContext = context;
  ...
  void dispose() {
    ...
    if (userId != -1) {
      Socketint().getInstance(userId.toString()); // fixme abubakar init socket
                ...
                Positioned(
                  child: StreamBuilder<QuerySnapshot>(
                      stream: firestore
                          ...
                          .where(newMessagesNumField, isGreaterThan: 0)
                          .snapshots(),
                      builder: (context, snapshot) {
                        if (!snapshot.hasData) return Container();
                        if (snapshot.data.docs.isEmpty) return Container();
                        int num = snapshot.data.docs.length;
                        return Container(
      ...
      onTap: (int index) {
        if (index == 1) {
          Navigator.of(context, rootNavigator: true).push(MaterialPageRoute(
            builder: (_) => MultiBlocProvider(providers: [
              BlocProvider(create: (_) => ChatsearchBloc()),
              BlocProvider(create: (_) => NearbyUsersBloc(context)),
            ], child: ChatPage()),
          ));
          return false;
        }
        // fixme abubakar home screen
        //  print("calllllllll:wahhhhh$index");

        var tappedTab = OBHomePageTabs.values[index];
        var currentTab = OBHomePageTabs.values[_lastIndex];

        if (tappedTab == OBHomePageTabs.refresher) {
          if (currentTab == OBHomePageTabs.timeline)
            _timelinePageController?.scrollToTop();
          if (currentTab == OBHomePageTabs.search)
            _searchPageController?.scrollToTop();
          return false; //we dont want to route to this page
        }
        //   print("calllllllll:wahhhhh11$tappedTab");

        if (tappedTab == OBHomePageTabs.timeline &&
            currentTab == OBHomePageTabs.timeline) {
          if (_timelinePageController.isFirstRoute()) {
            // print("calllllllll:if2");
        ...
        if (tappedTab == OBHomePageTabs.search &&
          ...
          if (_searchPageController.isFirstRoute()) {
            //  print("calllllllll:if");

            _searchPageController.scrollToTop();
          } else {
            //  print("calllllllll:else");
  ...
  void _onLoggedInUserChange(User newUser) async {
    if (newUser == null) {
      //Navigator.pushReplacementNamed(context, '/auth'); fixme abubakar change for unregisreruser
      Navigator.pushReplacementNamed(context, '/auth');
    } else {
      setCurrentUser(newUser);
      initNotifications();
      userId = newUser.id;
      fetchLotties(context);
      Socketint().getInstance(userId.toString()); // fixme abubakar init socket
...
enum OBHomePageTabs { timeline, search, refresher, menu, notifications }
