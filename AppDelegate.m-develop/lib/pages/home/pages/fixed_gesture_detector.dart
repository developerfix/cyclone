...
/// Signature used by [FixedDismissible] to indicate that it has been dismissed in
/// the given `direction`.
///

/// Signature used by [FixedDismissible] to give the application an opportunity to
/// confirm or veto a dismiss gesture.
///
/// Used by [FixedDismissible.confirmDismiss].
/// The direction in which a [FixedDismissible] can be dismissed.
...
/// non-null, the FixedDismissible widget animates its height (or width, whichever is
...
///
/// This sample shows how you can use the [FixedDismissible] widget to
...
/// is specified it is stacked behind the FixedDismissible's child and is exposed when
/// the child moves.
///
/// The widget calls the [onDismissed] callback either after its size has
/// collapsed to zero (if [resizeDuration] is non-null) or immediately after
/// the slide animation (if [resizeDuration] is null). If the FixedDismissible is a
/// list item, it must have a key that distinguishes it from the other items and
/// its [onDismissed] callback must remove the item from the list.
class FixedDismissible extends StatefulWidget {
  /// Creates a widget that can be dismissed.
  ///
  /// The [key] argument must not be null because [FixedDismissible]s are commonly
  /// used in lists and removed from the list when dismissed. Without keys, the
  /// default behavior is to sync widgets based on their index in the list,
  /// which means the item after the dismissed item would be synced with the
  /// state of the dismissed item. Using keys causes the widgets to sync
  /// according to their keys and avoids this pitfall.
  const FixedDismissible({
    @required Key key,
    @required this.child,
    this.background,
  ...
  /// FixedDismissible will begin upon the detection of a drag gesture. If set to
  ...
  @override
  _FixedDismissibleState createState() => _FixedDismissibleState();
}

class _FixedDismissibleClipper extends CustomClipper<Rect> {
  _FixedDismissibleClipper({
    @required this.axis,
  ...
  @override
  bool shouldReclip(_FixedDismissibleClipper oldClipper) {
...
class _FixedDismissibleState extends State<FixedDismissible>
  ...
  Widget build(BuildContext context) {
      ...
      assert(() {
        if (_resizeAnimation.status != AnimationStatus.forward) {
          ...
          throw FlutterError.fromParts(<DiagnosticsNode>[
            ErrorSummary(
                'A dismissed FixedDismissible widget is still part of the tree.'),
            ErrorHint(
              'Make sure to implement the onDismissed handler and to immediately remove the FixedDismissible '
    ...
    if (background != null) {
      content = Stack(children: <Widget>[
        if (!_moveAnimation.isDismissed)
          Positioned.fill(
            child: ClipRect(
              clipper: _FixedDismissibleClipper(
    ...
    return GestureDetector(
      ...
      onVerticalDragStart: _directionIsXAxis
          ? null
          : (_) {
      ...
      onVerticalDragEnd: _directionIsXAxis
          ? null
          : (_) {
              widget.onVerticalDragEnd(_);
              _handleDragEnd(_);
            },
      onVerticalDragCancel: widget.onVerticalDragCancel != null
