...
class OBTimelinePageState extends State<OBTimelinePage>
  ...
  UserStory _userStoryList;

  void autoUpdate() async {
    await _momentsStreamRefresher();
  ...
  void initState() {
    ...
    _userStoryList = new UserStory(
      myStories: [],
      stories: [],
    );

    SharedVariables.userStory = new UserStory(
      myStories: [],
      stories: [],
    );
    Future.delayed(Duration(seconds: 30)).then((value) => autoUpdate());
  }
        ...
        body: SingleChildScrollView(
          child: Column(children: <Widget>[
            ...
            Memories(
              ...
              userStoryList: _userStoryList,
            ),
            Container(
              height: height / 1.5,
              //child: !_loggedInUserBootstrapped fixme abubakar testing forunregistred user
              child: _loggedInUserBootstrapped
                  ? OBPostsStream(
                      ...
                      secondaryRefresher: _momentsStreamRefresher,
  ...
  void _onLoggedInUserChange(User newUser) async {
    ...
    UserStory stories = await _userService.getStory();
    print('STORIES LIST IS: $stories');
    setState(() {
      _loggedInUserBootstrapped = true;
      _initialPosts = initialPosts == null ? timelinePosts : initialPosts;
      _loggedInUserChangeSubscription.cancel();
      _userStoryList = stories;
      SharedVariables.userStory = stories;
    });
  }

  Future<UserStory> _momentsStreamRefresher() async {
    UserStory stories = await _userService.getStory();
    setState(() {
      _userStoryList = stories;
      SharedVariables.userStory = stories;
