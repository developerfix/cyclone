...
class MobilePodcastService extends PodcastService {
  ...
  Future<Podcast> loadPodcast({@required Podcast podcast, bool refresh}) async {
    if (podcast.id == null || refresh) {
      ...
      loadedPodcast.funding
          ?.forEach((f) => funding.add(Funding(url: f.url, value: f.value)));
  ...
  Future<psapi.Chapters> _loadChaptersByUrl(String url) {
    return compute<_FeedComputer, psapi.Chapters>(
  ...
  Future<psapi.Podcast> _loadPodcastFeed({@required String url}) {
    return compute<_FeedComputer, psapi.Podcast>(
