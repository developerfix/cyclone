...
import 'package:cyclone/Podcast/entities/persistable.dart';
...
import 'package:cyclone/Podcast/state/persistent_state.dart';
...
import 'audio_background_player.dart';
...
class MobileAudioPlayerService extends AudioPlayerService {
  ...
  final BehaviorSubject<PositionState> _playPosition =
  ...
  Future<void> playEpisode(
    ...
    log.info(
        'Playing episode ${episode?.guid} - ${episode?.title} - ${episode?.id}');
    ...
    if (episode.guid != '') {
      ...
      await AudioService.customAction(
          'track', _loadTrackDetails(uri, startPosition));
      ...
      try {
        ...
        if (streaming && _episode.hasChapters) {
          ...
          _episode.chapters =
              await podcastService.loadChaptersByUrl(url: _episode.chaptersUrl);
  ...
  Future<void> seek({int position}) async {
    ...
    _playPosition
        .add(PositionState(p, duration, complete.toInt(), _episode, true));
  ...
  Future<Episode> resume() async {
    ...
    if (_episode == null) {
      ...
      if (AudioService.currentMediaItem == null) {
        ...
        log.fine(
            ' - media item exists loading item id ${AudioService.currentMediaItem?.id ?? -1}');
        _episode = await repository
            .findEpisodeById(int.parse(AudioService.currentMediaItem.id));
      }
    } else {
  ...
  Future<void> _onUpdatePosition() async {
    ...
    if (playbackState != null) {
      ...
      var complete = position.inSeconds > 0
          ...
          : 0;
      var buffering = AudioService.playbackState.processingState ==
  ...
  void updateCurrentPosition(Episode e) {
    if (e != null) {
      ...
      _playPosition.add(PositionState(Duration(milliseconds: e.position),
          duration, complete.toInt(), e, false));
    }
  }
  ...
  Future<void> _start() async {
    ...
    await AudioService.start(
      ...
      androidNotificationColor:
          androidNotificationColor?.value ?? Colors.orange.value,
  ...
  Future<void> _savePosition() async {
    ...
    if (_episode != null) {
      ...
      log.fine(
          '_savePosition(): Current position is $currentPosition - stored position is ${_episode.position}');
  ...
  void _updateChapter(int seconds, int duration) {
    if (_episode == null) {
      ...
      for (var x = 0; x < _episode.chapters.length; x++) {
        ...
        final endTime = x == (_episode.chapters.length - 1)
            ? duration
            : chapters[x + 1].startTime;
